...
TEST_CASE("Given a std::vector<A> …", "[fmap]") {
  ...
  SECTION(
      "… fmap given only a function f : A → B, should produce a function "
  ...
  SECTION(
      "… it should satisfy the composition relation: fmap(g, fmap(f, a)) "
      "== fmap(g∘f, a)") {
...
TEST_CASE(
    "For std::vector, calling fmap should induce the same constructor "
...
TEST_CASE(
    "For std::list, calling fmap should induce the same constructor and "
...
TEST_CASE(
    "The std::array typeconstructor will not be a regular functor,  "
...
TEST_CASE(
    "For std::array, calling fmap should induce the same constructor and "
...
TEST_CASE(
    "For std::deque, calling fmap should induce the same constructor "
    "and assignment calls as simply using std::transform.",
    "[CtorAs], [fmap]") {
...
TEST_CASE(
    "Given a function f : A → B, fmap should produce a function object "
